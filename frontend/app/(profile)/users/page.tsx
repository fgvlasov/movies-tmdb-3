    ...
    <div className="container">
      ...
      <div>
        ...
        {users && users.map((user: any) => (
          <div className="grid grid-cols-3 border-b p-2 items-center" key={user.id}>
              <div className="flex w-full truncate text-wrap justify-start p-2 text-xs md:text-base">Email: {user.email}</div>
              <div className="flex w-full border-l truncate text-wrap justify-star p-2 text-xs md:text-base">Admin: {user.is_admin ? "Yes" : "No"}</div>
                ...
                {user.is_admin
                  ...
                  <form action={changeUserAdmin} className="flex items-center">
                    ...
                    <Button className="flex gap-1" variant="ghost">
                      <ShieldOff className="h-5 w-5" />Revoke "Admin" {user.is_admin}
                    </Button>
                  </form>
                  :
                  <form action={changeUserAdmin} className="flex items-center">
